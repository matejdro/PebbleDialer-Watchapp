...
#define MENU_Y_OFFSET (PBL_IF_RECT_ELSE(9 + STATUS_BAR_LAYER_HEIGHT, 27))
...
bool actions_menu_got_data(uint8_t packetId, DictionaryIterator* dictionary)
{
	if (packetId != 0)
		return false;
...
void actions_menu_init(void)
{
	...
	#ifdef PBL_COLOR
		menu_layer_set_highlight_colors(menu, GColorJaegerGreen, GColorBlack);
	#endif
}
